...
async def cost_menu_getter(dialog_manager: DialogManager, **kwargs):
    session: DataInteraction = dialog_manager.middleware_data.get('session')
...
async def get_cost(msg: Message, widget: ManagedTextInput, dialog_manager: DialogManager, text: str):
    try:
        cost = int(text)
    except Exception:
        await msg.answer('Новая стоимость должна быть числом, пожалуйста попробуйте еще раз')
        return
    session: DataInteraction = dialog_manager.middleware_data.get('session')
    await session.set_price(cost)
    await dialog_manager.switch_to(adminSG.cost_menu)
