...
admin_dialog = Dialog(
    Window(
        ...
        Column(
            ...
            SwitchTo(Const('Управление ценой'), id='cost_menu_switcher', state=adminSG.cost_menu),
    ...
    Window(
        Format('Действующая цена на выставление анкеты: {cost}'),
        Const('Для смены цены введите новую стоимость ниже:'),
        TextInput(
            id='get_cost',
            on_success=getters.get_cost
        ),
        getter=getters.cost_menu_getter,
        state=adminSG.cost_menu
    ),
    Window(
